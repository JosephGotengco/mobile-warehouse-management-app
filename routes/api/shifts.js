...
router.post("/", isLoggedIn, async (req, res) => {
  try {
    ...
    if (!isValidTime(startTime))
      return res.status(400).send("Please provide a valid start time.");
    if (!isValidTime(endTime))
      return res.status(400).send("Please provide a valid end time.");
    if (hmsToSeconds(endTime) - hmsToSeconds(startTime) < 0)
    ...
    if (sameDay(nd, new Date(fullYear, month - 1, dateNum)))
      return res
        .status(400)
        .send("Please schedule shifts a day before the shift date.");
    ...
    res
      .status(200)
      .send(
        `Successfully added a shift from ${startTime} to ${endTime} on ${date}`
      );
  } catch (e) {
    console.log("error", e);
  }

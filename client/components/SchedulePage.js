...
class SchedulePage extends Component {
  ...
  componentDidMount() {
    ...
    var currentDate = `${nd.getFullYear()}-${nd.getMonth() +
      1}-${nd.getDate()}`;
    console.log(currentDate);
    let selectedShifts = Object.values(shifts).filter(
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      Object.keys(prevProps.user.shifts).length !==
      Object.keys(this.props.user.shifts).length
    ) {
      // load data for markedDates and agendaDates
      let markedDates = {};
      let agendaDates = {};
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          <Text
            style={{ fontFamily: "Rubik-Bold", fontSize: 18, color: "#333333" }}
          >
            {todayOrDate}
            {"\n"}
            ...
            if (i + 1 === length) {
              return (
                <View key={i} style={{ ...styles, marginBottom: 40 }}>
                  ...
                  >
                    {startHourNormal}:{startMinute}
                    {startMeridiem}-{endHourNormal}:{endMinute}
                    {endMeridiem}
                  </Text>
                  <Text
                    style={{
                  ...
                  >
                    {hourDifference > 0 ? `${hourDifference} hours` : null}
                    {minuteDifference > 0
